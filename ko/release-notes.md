...
* [Android] 1.13.3.0
    * 네이티브 라이브러리(.so) 무결성 검증 활성화
        * Google Play 자동 무결성 보호 적용 시 오탐이 발생하는 문제를 수정하였습니다.
    * 앱 증명 개선
        * 네이티브 라이브러리(.so) 검증 기능을 추가하였습니다.
        * 앱 증명 기능의 최소지원 버전이 Android 1.13.3.0, SDK 0.5.0 으로 변경되었습니다.
    * Java 난독화 개선
        * 이스케이프 문자열 처리 시 발생하던 난독화 오류를 수정하였습니다.
        * 런타임 환경에서 특정 클래스에 대해 VerifyError가 발생하는 문제를 수정하였습니다.
        * 보호 작업 수행 시 난독화 검증 단계를 추가하였습니다.
            * CLI에서 `--verify-obfuscated-dex` 옵션을 추가하면 검증이 활성화됩니다.
    * 버그 수정
        * Android 5.1(32bit) 에뮬레이터에서 크래시가 발생하는 문제를 수정하였습니다.
* [iOS] 1.5.4
    * 앱 증명 명칭 변경 사항 적용
    * 보안성 강화
	* 라이브러리 변조 탐지 개선
 	    * 일부 구형 iOS에서 라이브러리 변조 탐지가 안되는 문제를 수정하였습니다.
    * 보호 작업 성능 개선
        * 보호 작업 성능을 개선하였습니다.   
 ...
 * [Native Code Obfuscator] 1.1.0
    * 기능 개선
        * 문자열 인코딩 설정 방식을 간소화하였습니다.
        * NDK 툴체인 설치 방식을 변경하였습니다.
