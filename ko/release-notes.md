...
* [Android] 1.14.0.0
    * DEX 암호화 대상 지정 기능 추가
        * 패키지 또는 클래스 단위로 DEX 암호화 대상을 선택할 수 있는 기능을 추가했습니다.
        * 자세한 내용은 **Android Developer's Guide > 3. 통합 설정 파일 > 3.2 기능별 설정 > DEX 암호화 대상 지정**을 참고하세요.
    * 통합 설정 파일 지원
        * 리소스 문자열 난독화 규칙과 DEX 암호화 범위를 하나의 통합 설정 파일에서 관리할 수 있도록 개선했습니다.
        * 자세한 내용은 **Android Developer's Guide > 3. 통합 설정 파일**을 참고하세요.
    * Android ID 기반 블랙리스트 기능 추가
        * Android ID를 기준으로 특정 단말을 블랙리스트에 등록할 수 있는 기능을 추가했습니다.
    * 에뮬레이터 탐지 패턴 추가
        * UgPhone 에뮬레이터 탐지 패턴을 추가했습니다.
    * 버그 수정
        * MyCard 결제 시 앱이 비정상적으로 종료되는 문제를 수정했습니다.
        * 원격 제어 앱 탐지 시 발생하는 오탐을 수정했습니다.
* [iOS] 1.6.0
    * iOS 최소 지원 버전 상향
        * 최소 지원 버전을 iOS 13으로 상향했습니다.
    * 안정성 개선
* [Unity] 0.5.2
    * iOS 환경에서 SecureValue 변조 시 차단이 정상적으로 동작하지 않는 문제를 수정했습니다.
    * iOS SDK 1.6.0 업데이트
* [Unreal] 0.2.11
    * iOS SDK 1.6.0 업데이트
* [Console]
    * Android 통합 설정 파일 업로드 기능 추가
        * Android 앱 보호 작업에 사용할 통합 설정 파일을 업로드할 수 있는 기능을 추가했습니다.
    * Android ID 기반 블랙리스트 차단 기능 추가
        * 블랙리스트에서 Android ID를 기준으로 특정 단말을 차단할 수 있도록 기능을 추가했습니다.
...
* 공통
    * '무결성 인증' 기능 명칭을 '앱 증명'으로 변경
        * SDK 인터페이스가 앱 증명(App Attestation) 명칭에 맞게 변경되었습니다.
        * CLI 옵션이 `--integrity`에서 `--app-attestation`으로 변경되었습니다.
* [Android] 1.13.3.0
    * 네이티브 라이브러리(.so) 무결성 검증 활성화
        * Google Play 자동 무결성 보호 적용 시 오탐이 발생하는 문제를 수정했습니다.
    * 앱 증명 개선
        * 네이티브 라이브러리(.so) 검증 기능을 추가했습니다.
        * 앱 증명 기능의 최소 지원 버전이 Android 1.13.3.0, SDK 0.5.0으로 변경되었습니다.
    * Java 난독화 개선
        * 이스케이프 문자열 처리 시 발생하던 난독화 오류를 수정했습니다.
        * 런타임 환경에서 특정 클래스에 대해 VerifyError가 발생하는 문제를 수정했습니다.
        * 보호 작업 수행 시 난독화 검증 단계를 추가했습니다.
            * CLI에서 `--verify-obfuscated-dex` 옵션을 추가하면 검증이 활성화됩니다.
    * 버그 수정
        * Android 5.1(32bit) 에뮬레이터에서 크래시가 발생하는 문제를 수정했습니다.
* [iOS] 1.5.4
    * 앱 증명 명칭 변경 사항 적용
    * 보안성 강화
    * 라이브러리 변조 탐지 개선
        * 일부 구형 iOS에서 라이브러리 변조가 탐지되지 않는 문제를 수정했습니다.
    * 보호 작업 성능 개선
* [Unity] 0.4.30
    * iOS SDK 1.5.4 업데이트
* [Unreal] 0.2.8
...
* [Console]
    ...
    * 앱 증명 기능 추가
        * ABI별 해시 관리 기능 추가
        * 검증 모드 자동 전환 기능 추가
    * 블랙리스트 차단 사유 오표출 버그 수정
* [Native Code Obfuscator] 1.1.0
    * 기능 개선
        * 문자열 인코딩 설정 방식을 간소화했습니다.
        * NDK 툴체인 설치 방식을 변경했습니다.
...
* [Android] 1.13.2.1
    * 본 버전은 사용 중단된 1.13.0.0, 1.13.1.0, 1.13.2.0 버전의 변경 사항을 포함합니다.
    * 특정 환경에서 NHN AppGuard 엔진 로딩 실패로 인해 크래시가 발생하는 문제 수정
    * 네이티브 라이브러리(.so)의 무결성 검증 비활성화

<a id="may-12-2026"></a>
### 2026. 05. 12. { #may-12-2026 }
* [Android] 1.13.2.0 **(사용 중단 권고)**
    * 무결성 인증 개선
        * 서명 검증 대상을 라이브러리 Split APK까지 확장했습니다.
    * 네이티브 라이브러리 변조 탐지 기능 추가
        * 네이티브 라이브러리(.so)의 무결성을 검증하여, 변조된 경우 이를 탐지하는 기능을 추가했습니다.
    * KernelSU 루팅 탐지 기능 추가
        * KernelSU 기반 루팅 디바이스를 탐지할 수 있도록 기능을 추가했습니다.
    * 원격 제어 탐지 개선
        * 원격 제어 탐지 방식을 기존의 앱 설치 여부 기준에서 실제 실행 여부 기준으로 개선했습니다.
...
* [Android] 1.13.1.0 **(사용 중단 권고)**
    * 비신뢰 환경 탐지 기능 추가
        * 기기의 보안 상태를 확인하여 비신뢰 환경을 탐지합니다.
    * Java 난독화 버그 수정
        * 1.13.0.0 버전에서 난독화가 일부만 적용되던 문제를 수정했습니다.
* [iOS] 1.5.1
    * 무결성 인증 버그 수정
        * 특정 상황에서 검증을 실패하는 문제를 수정했습니다.
* [Unity] 0.4.27
	* (iOS) iOS SDK 1.5.1 업데이트
* [Unreal] 0.2.5
	* (iOS) iOS SDK 1.5.1 업데이트
* [Console]
    * 대시보드의 이상 행위 탐지 현황에 비신뢰 환경(Android) 추가
...
* [Android] 1.13.0.0 **(사용 중단 권고)**
    * 무결성 인증 신규 기능 추가
    	...
    	* 기능을 활성화하려면 NHN Cloud 홈페이지의 [고객지원 > 문의하기](https://www.nhncloud.com/kr/support/inquiry)를 이용해 주시기 바랍니다.
...
* [Console]
    * 무결성 인증 신규 기능 추가
        * 상품 탭에서 무결성 검증 정책, Android 서명 정보 등을 확인할 수 있습니다.
        * 앱 탭에서 앱별 앱 서명, 해시, 키값을 확인할 수 있고, 해시 수집 모드를 변경할 수 있습니다.
        * 검증 로그 탭에서 무결성 검증 로그를 확인할 수 있습니다.
        * 기능을 활성화하려면 NHN Cloud 홈페이지의 [고객지원 > 문의하기](https://www.nhncloud.com/kr/support/inquiry)에서 활성화를 요청하세요.
    * 난독화 요청 모달 개선
        * 리소스 난독화 기능 추가에 따라, 난독화 요청 모달에서 리소스 난독화 옵션이 추가되었습니다.
        * 네이티브 코드 난독화 라이선스 요청 옵션이 추가되었습니다.
    * 콘솔 보호 작업 시 리소스 난독화 파일을 첨부할 수 있도록 리소스 난독화 규칙 파일 첨부 기능 추가

<a id="march-10-2026"></a>
### 2026. 03. 10. { #march-10-2026 }
* [Android] 1.12.4.18
    * APK 패키징 도구를 업데이트하여 16KB 페이지 사이즈를 지원하도록 개선
* [iOS] 1.4.16
	* rootless 탈옥 환경 보안성 개선
* [Unity] 0.4.25
	* (iOS) iOS SDK 1.4.16 업데이트
* [Unreal] 0.2.3
	* (iOS) iOS SDK 1.4.16 업데이트
...
* [Android] 1.12.4.17
    * Unity, Unreal 단일 아키텍처 앱 번들 종료 문제 수정

<a id="january-27-2026"></a>
### 2026. 01. 27. { #january-27-2026 }
* [Android] 1.12.4.16
	* React Native 0.79.0 이상 앱 호환성 개선
* [iOS] 1.4.15
	* 콘솔 업로드 보호 시 서명 변조 탐지 개선
      * CLI를 통한 보호뿐 아니라 콘솔 업로드 보호 시에도 서명 변조 탐지가 가능합니다.
* [Unity] 0.4.24
    * (iOS) iOS SDK 1.4.15 업데이트
* [Unreal] 0.2.2
    * (iOS) iOS SDK 1.4.15 업데이트
* [Console]
    * 대시보드의 이상 행위 탐지 추이에 OS 조건이 적용되지 않는 버그 수정
    * 대시보드의 이상 행위 탐지 현황에 매크로툴(iOS), VPN(Android, iOS), 직장 프로필(Android) 추가
    * 대시보드 조회 시 차트별로 로딩 표기
...
* [iOS] 1.4.14
    * 암호화 API 지원 추가
    * PDF 사용자 가이드를 HTML로 변경
* [Unity] 0.4.23
    * (iOS) iOS SDK 1.4.14 업데이트
* [Unreal] 0.2.1
    * (iOS) iOS SDK 1.4.14 업데이트
* [Console]
    * 기본 정책에 잘못된 정책이 표출되는 현상 수정
...
* [Android] 1.12.4.10
    ...
    * 앱 보호 작업 성능 개선
* [Unity] 0.4.21
    * (Android) Android SDK 0.3.0 업데이트

<a id="july-29-2025"></a>
### 2025. 07. 29. { #july-29-2025 }
* [Android] 1.12.4.9
    * 16KB 메모리 페이지 크기 지원
    * Business 플랜 적용 시, Application.onCreate()에서 유저 아이디가 정상 등록되지 않던 문제 수정
    * Fly GPS와 Mock Locations 탐지 콜백 코드가 정상적으로 들어오지 않는 문제 수정
    * 갤럭시 S6 디바이스에서 비정상 종료되는 문제 수정
* [iOS] 1.4.12
    * Rootless 탈옥 환경 탐지 개선
* [Unity] 0.4.20
    * (iOS) iOS SDK 1.4.12 업데이트

* [Console]
    * 개별 차단 모달 개선
        * 기본 정책을 직관적으로 이해할 수 있도록 UI 개선
        * 간헐적으로 개별 차단 항목이 중복되어 표출되는 버그 수정
