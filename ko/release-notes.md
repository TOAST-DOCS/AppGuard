...
* [Android] 1.13.2.0
    * 무결성 인증 개선
        * 서명 검증 대상을 라이브러리 Split APK까지 확장되었습니다.
    * 네이티브 라이브러리 변조 탐지 기능 추가
        * 네이티브 라이브러리(.so)의 무결성을 검증하여, 변조된 경우 이를 탐지하는 기능을 추가하였습니다.
    * KernelSU 루팅 탐지 기능 추가
        * KernelSU 기반 루팅 디바이스를 탐지할 수 있도록 기능을 추가하였습니다.
    * 원격제어 탐지 개선
        * 원격 제어 탐지 방식을 기존의 앱 설치 여부 기준에서 실제 실행 여부 기준으로 개선하였습니다.
...
* [iOS] 1.5.2
  ...
  * 변조 탐지 개선
    * 앱 번들 내부 동적 라이브러리(framework, dylib)변조 탐지 기능이 추가 되었습니다.
