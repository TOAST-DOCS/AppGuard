...
- '조건 차단'시 설정한 조건을 만족한 경우 블랙리스트에 등록되며, 해당 사용자가 앱을 실행한 경우 NHN AppGuard 안내 창이 나타나고 앱이 종료됩니다.
...
- NHN AppGuard 정책 > 전체 차단
    ...
    - 전체 차단으로 설정된 이상 행위 없이 앱을 재실행하면 앱을 정상적으로 사용할 수 있습니다.
...
- NHN AppGuard 정책 > 조건 차단
    ...
    - 조건의 만족 여부는 작일 00:00:00 ~ 23:59:59 까지 발생한 탐지 로그를 기반으로 판단하며, 각 조건은 종류별로 독립적으로 동작합니다.
    - 조건 차단으로 등록된 블랙리스트는 NHN AppGuard 블랙리스트 탭에서 확인할 수 있습니다.
    - 조건 차단으로 등록된 블랙리스트로 인해 차단된 사용자는 NHN AppGuard 대시보드 탭에 표출되지 않습니다.
    - 조건 차단으로 블랙리스트에 등록된 사용자는 이상 행위 없이 앱을 실행하더라도 설정한 차단 기간 동안 앱을 사용할 수 없습니다.
    - 조건 차단으로 설정 또는 조건을 변경한 경우 약 10분 후 모든 사용자에 적용됩니다.
