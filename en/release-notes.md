...
### 2023. 12. 19.
* [iOS] 1.3.13
    * 엔진 보안성이 강화 되었습니다.(엔진 자가보호)
    * AppKey 설정 전 탐지 시 로그가 누락되는 문제가 개선 되었습니다.(보호작업을 적용 시 적용)
    * 일부 클래스 심볼 난독화 시 보호작업에 실패하는 이슈가 개선 되었습니다.
    * CLI에서 `--latestVersion`인자가 기본으로 적용됩니다.

* [Android] 1.10.4.2
    * Android 14 대응
        * Android 14(API 34)이상을 타겟팅하는 앱에서 앱가드 적용 시 비정상 종료될 수 있는 문제를 수정합니다.
    * 멀티프로세스 대응
        * 애플리케이션 구성 요소에 android:process 특성이 있을 때 간헐적으로 비정상 종료되는 오류를 수정합니다.
    * armeabi 아키텍처 이슈 수정
        * armeabi 아키텍처 만을 사용하는 앱에서 발생하는 비정상 종료 오류를 수정합니다.
* [Unity] 
    * [iOS] NHN AppGuard iOS 초기화 작업 개선
        * Diresu.D 함수는 이제 사용되지 않습니다 (deprecated).
* [Gradle Plugin] 1.2.1
    * 서명 키 비밀번호 설정 오류 수정
        * 스토어 비밀번호와 키 비밀번호가 다를 경우 서명에 실패하는 오류를 수정합니다.
